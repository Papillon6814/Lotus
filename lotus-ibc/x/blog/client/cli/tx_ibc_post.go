...
func CmdSendIbcPost() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "send-ibc-post [src-port] [src-channel] [title] [contenet]",
		Short: "Send a ibcPost over IBC",
		Args:  cobra.ExactArgs(4),
		RunE: func(cmd *cobra.Command, args []string) error {
			argsTitle := string(args[2])
			argsContenet := string(args[3])
			...
			msg := types.NewMsgSendIbcPost(sender, srcPort, srcChannel, timeoutTimestamp, string(argsTitle), string(argsContenet))
